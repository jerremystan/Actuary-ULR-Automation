...
Paid_function <- function(cohort, term, sheet, tahun, kuarter){   #fungsi term, cohort, sheet, tahun, kuarter
  ...
  num_col <- lapply(1:row_count, function(x){
    ...
    ifelse(length(sql_data$Gross[sql_data$LossYear == tahun & sql_data$LossMonth == kuarter & sql_data$ReportingYear == df_index$RPTYear[df_index$ColIndex == x] & sql_data$ReportingMonth == df_index$RPTQuar[df_index$ColIndex == x] & sql_data$Cohort == cohort & sql_data$IsShortTerm == term & sql_data$SheetName == sheet]) > 0, sql_data$Gross[sql_data$LossYear == tahun & sql_data$LossMonth == kuarter & sql_data$ReportingYear == df_index$RPTYear[df_index$ColIndex == x] & sql_data$ReportingMonth == df_index$RPTQuar[df_index$ColIndex == x] & sql_data$Cohort == cohort & sql_data$IsShortTerm == term & sql_data$SheetName == sheet],"")
...
template_prior <- read_excel("Workbook/PaidClaim-Gross.Prior.xlsx",sheet = "Auto", range = excel_range)
...
Triangle_raw_function <- function(sheet,cohort,term){
  triangle_temp <- read_excel(paste0("Workbook/PC ",cohort,"-",term,"/PaidClaim-Gross.Raw.xlsx"), sheet = sheet, range = excel_range)
...
Triangle_prior_function <- function(sheet){
  triangle_temp <- read_excel(paste0("Workbook/","/PaidClaim-Gross.Prior.xlsx"), sheet = sheet, range = excel_range)
  for (x in 1:row_count){
    if (x > 1){
      triangle_temp[x,(row_count+2+2-x):(row_count+2)] <- NA
    }
...
for ( i in cohort_start:cohort_end){ #COHORT
  ...
  for (j in 0:1){ #TERM
    ...
    for (k in sheet_df$Sheet){ #SHEET 
      ...
      Paid_data <- data.frame() #NEW BLANK temporary every LoB
      ...
      for (l in 2009:val_year){ #ACCIDENT YEAR
        ...
        for (m in 1:4){ #ACCIDENT QUARTER
          ...
          if(l == val_year & m > val_quarter){break}
            ...
            result_row <- Paid_function(i,j,k,l,m)
            Paid_data <- rbind(Paid_data, result_row)
          }
      ...
      Paid_data[,3:ncol(Paid_data)] <- lapply(Paid_data[,3:ncol(Paid_data)], as.numeric)
      colnames(Paid_data) <- c("Year","Kuarter",seq(1,nrow(Paid_data)))
      writeData(wb, sheet_name, Paid_data)
      #addStyle(wb, sheet = sheet_name, style = createStyle(numFmt = "######.######"), rows = 1:nrow(Paid_data)+1, cols = 1:ncol(Paid_data), gridExpand = TRUE)
      
    } #SHEET 
    
    saveWorkbook(wb, paste0(dir_name,"/PaidClaim-Gross.Raw.xlsx"), overwrite = TRUE)
  ...
  if(i == 2020){
    ...
    prior_auto <- template_prior
    prior_auto[,3:(row_count + 2)] <- prior_auto_st[,3:(row_count + 2)] + prior_auto_lt[,3:(row_count + 2)]
    ...
    prior_auto_PL <- template_prior
    prior_auto_PL[,3:(row_count + 2)] <- prior_auto_PL_st[,3:(row_count + 2)] + prior_auto_PL_lt[,3:(row_count + 2)]
    ...
    prior_auto_TLM <- template_prior
    prior_auto_TLM[,3:(row_count + 2)] <- prior_auto_TLM_st[,3:(row_count + 2)] + prior_auto_TLM_lt[,3:(row_count + 2)]
    ...
    prior_auto_TLN <- template_prior
    prior_auto_TLN[,3:(row_count + 2)] <- prior_auto_TLN_st[,3:(row_count + 2)] + prior_auto_TLN_lt[,3:(row_count + 2)]
    ...
    prior_auto_TP <- template_prior
    prior_auto_TP[,3:(row_count + 2)] <- prior_auto_TP_st[,3:(row_count + 2)] + prior_auto_TP_lt[,3:(row_count + 2)]
    ...
    prior_eng <- template_prior
    prior_eng[,3:(row_count + 2)] <- prior_eng_st[,3:(row_count + 2)] + prior_eng_lt[,3:(row_count + 2)]
    ...
    prior_fire[,3:(row_count + 2)] <- prior_fire_st[,3:(row_count + 2)] + prior_fire_lt[,3:(row_count + 2)]
    ...
    prior_marine[,3:(row_count + 2)] <- prior_marine_st[,3:(row_count + 2)] + prior_marine_lt[,3:(row_count + 2)]
    ...
    prior_various[,3:(row_count + 2)] <- prior_various_st[,3:(row_count + 2)] + prior_various_lt[,3:(row_count + 2)]
    ...
    prior_pa[,3:(row_count + 2)] <- prior_pa_st[,3:(row_count + 2)] + prior_pa_lt[,3:(row_count + 2)]
    ...
    prior_liab <- template_prior
    prior_liab[,3:(row_count + 2)] <- prior_liab_st[,3:(row_count + 2)] + prior_liab_lt[,3:(row_count + 2)]
    ...
    prior_health <- template_prior
    prior_health[,3:(row_count + 2)] <- prior_health_st[,3:(row_count + 2)] + prior_health_lt[,3:(row_count + 2)]
    ...
    prior_vare[,3:(row_count + 2)] <- prior_vare_st[,3:(row_count + 2)] + prior_vare_lt[,3:(row_count + 2)]
    ...
    prior_firexl <- template_prior
    prior_firexl[,3:(row_count + 2)] <- prior_firexl_st[,3:(row_count + 2)] + prior_firexl_lt[,3:(row_count + 2)]
    ...
    saveWorkbook(wbp, paste0("Workbook/PC ",i,"-ST/PaidClaim-Gross.Prior.xlsx"),overwrite = TRUE)
    ...
    saveWorkbook(wbp, paste0("Workbook/PC ",i,"-LT/PaidClaim-Gross.Prior.xlsx"),overwrite = TRUE)

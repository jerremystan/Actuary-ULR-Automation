...
sql_data <- sql_data %>% group_by(LineOfBusiness, Tahun, Kuarter) %>% summarize(Gross = sum(Gross),Net = sum(Net))
...
EP_function <- function(tahun,quarter){
  ...
  new_row <- data.frame(`Year` = tahun,
                        `Quarter` = quarter,
                        `Gross_Auto` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "AUTO" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0 ,sql_data$Gross[sql_data$LineOfBusiness == "AUTO" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_Marine-Ecommerce` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "MARINE-ECOMMERCE" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "MARINE-ECOMMERCE" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter], 0),
                        `Gross_Marine-Others` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "MARINE-OTHERS" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) >0 ,sql_data$Gross[sql_data$LineOfBusiness == "MARINE-OTHERS" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter], 0),
                        `Gross_Marine-Others-XL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "MARINE-OTHERS-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "MARINE-OTHERS-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter], 0),
                        `Gross_Fire` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "FIRE" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0 ,sql_data$Gross[sql_data$LineOfBusiness == "FIRE" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter], 0),
                        `Gross_Engineering` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "ENGINEERING" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "ENGINEERING" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_Liability` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "LIABILITY" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "LIABILITY" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_PA-RETAIL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "PA-RETAIL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "PA-RETAIL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_PA-COMMERCIAL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "PA-COMMERCIAL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "PA-COMMERCIAL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_PA-TRAVEL-RETAIL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "PA-TRAVEL-RETAIL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "PA-TRAVEL-RETAIL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_PA-TRAVEL-COMMERCIAL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "PA-TRAVEL-COMMERCIAL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "PA-TRAVEL-COMMERCIAL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_PA-XL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "PA-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "PA-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_VARIOUS-FIRE` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-FIRE" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-FIRE" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_VARIOUS-LIABILITY` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-LIABILITY" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-LIABILITY" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_VARIOUS-PA-RETAIL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-PA-RETAIL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-PA-RETAIL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_VARIOUS-PA-COMMERCIAL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-PA-COMMERCIAL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-PA-COMMERCIAL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_VARIOUS-TRADE CREDIT` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-TRADE CREDIT" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0, sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-TRADE CREDIT" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_VARIOUS-TRAVEL-RETAIL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-TRAVEL-RETAIL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-TRAVEL-RETAIL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_VARIOUS-TRAVEL-COMMERCIAL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-TRAVEL-COMMERCIAL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-TRAVEL-COMMERCIAL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_VARIOUS-TRAVEL-XL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-TRAVEL-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-TRAVEL-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_VARIOUS-OTHERS` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-OTHERS" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-OTHERS" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        `Gross_VARIOUS-OTHERS-XL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-OTHERS-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-OTHERS-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        ...
                        `Gross_HEALTH` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "HEALTH" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "HEALTH" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        ...
                        `Gross_VARIOUS-ECOMMERCE` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-ECOMMERCE" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-ECOMMERCE" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        ...
                        `Gross_MARINE-ECOMMERCE-XL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "MARINE-ECOMMERCE-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "MARINE-ECOMMERCE-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        ...
                        `Gross_VARIOUS-ECOMMERCE-XL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-ECOMMERCE-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "VARIOUS-ECOMMERCE-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        ...
                        `Gross_AUTO-XL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "AUTO-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "AUTO-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        ...
                        `Gross_ENGINEERING-XL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "ENGINEERING-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "ENGINEERING-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        ...
                        `Gross_FIRE-XL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "FIRE-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "FIRE-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        ...
                        `Gross_TRADE CREDIT-XL` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "TRADE CREDIT-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "TRADE CREDIT-XL" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
                        ...
                        `Gross_LBLW` = ifelse(length(sql_data$Gross[sql_data$LineOfBusiness == "LBLW" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter]) > 0,sql_data$Gross[sql_data$LineOfBusiness == "LBLW" & sql_data$Tahun == tahun & sql_data$Kuarter == quarter],0),
...
saveWorkbook(wb, paste0(dir_name,"/Earned Premium.Total.Raw.xlsx"), overwrite = TRUE)

#PRIOR FOR ALL COHORT
...
EP_prior_final_ST <- EP_data_ST_prior
...
wb <- createWorkbook()
addWorksheet(wb, "EP")
# writeData(wb, "EP", vector_gn_prior)
writeData(wb, "EP", EP_prior_final_ST, startCol = 1, startRow = 2)
saveWorkbook(wb, "Workbook/Earned Premium.Total.Prior.xlsx", overwrite = TRUE)

...
template_prior <- read_excel("Workbook/OSClaim-Gross.Prior.xlsx",sheet = "Auto", range = excel_range)
...
Triangle_raw_function <- function(sheet,cohort,term){
  triangle_temp <- read_excel(paste0("Workbook/OS ",cohort,"-",term,"/OSClaim-Gross.Raw.xlsx"), sheet = sheet, range = excel_range)
...
Triangle_prior_function <- function(sheet){
  triangle_temp <- read_excel(paste0("Workbook/","/OSClaim-Gross.Prior.xlsx"), sheet = sheet, range = excel_range)
  for (x in 1:row_count){
    if (x > 1){
      triangle_temp[x,(row_count+2+2-x):(row_count+2)] <- NA
    }
...
for ( i in cohort_start:cohort_end){ #COHORT
  ...
  for (j in 0:1){ #TERM
    ...
    saveWorkbook(wb, paste0(dir_name,"/OSClaim-Gross.Raw.xlsx"), overwrite = TRUE)
  ...
  #   
    ...
    saveWorkbook(wbp, paste0("Workbook/OS ",i,"-ST/OSClaim-Gross.Prior.xlsx"),overwrite = TRUE)
    ...
    saveWorkbook(wbp, paste0("Workbook/OS ",i,"-LT/OSClaim-Gross.Prior.xlsx"),overwrite = TRUE)

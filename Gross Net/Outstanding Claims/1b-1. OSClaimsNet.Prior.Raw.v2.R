...
template_prior <- read_excel("Workbook/OSClaim-Net.Prior.xlsx",sheet = "Auto", range = excel_range)
...
Triangle_raw_function <- function(sheet,cohort,term){
  triangle_temp <- read_excel(paste0("Workbook/OS ",cohort,"-",term,"/OSClaim-Net.Raw.xlsx"), sheet = sheet, range = excel_range)
...
Triangle_prior_function <- function(sheet){
  triangle_temp <- read_excel(paste0("Workbook/","/OSClaim-Net.Prior.xlsx"), sheet = sheet, range = excel_range)
  for (x in 1:row_count){
    if (x > 1){
      triangle_temp[x,(row_count+2+2-x):(row_count+2)] <- NA
    }
...
for ( i in cohort_start:cohort_end){ #COHORT
  ...
  # if(i == 2020){
    ...
    saveWorkbook(wbp, paste0("Workbook/OS ",i,"-ST/OSClaim-Net.Prior.xlsx"),overwrite = TRUE)
    ...
    saveWorkbook(wbp, paste0("Workbook/OS ",i,"-LT/OSClaim-Net.Prior.xlsx"),overwrite = TRUE)

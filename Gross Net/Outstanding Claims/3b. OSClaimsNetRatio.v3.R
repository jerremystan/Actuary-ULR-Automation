...
OS_prior_function <- function(cohort, term, sheet_name){
...
'    #MENGAMBIL DF per Cohort (Partial)
    path_name <- paste0("Workbook/OS ",cohort,st_or_lt,"/OSClaim-Net.Raw.xlsx")
    ...
    data_comp_M <- read_excel("Workbook/OSClaim-Net.Prior.xlsx", sheet = "Marine")
    data_comp <- data_comp_M
    
    #MENGAMBIL DF Total (semua cohort)
    path_tot <- "Workbook/OSClaim-NetTotal.Raw.FIXED-LBLW.v1.xlsx"
...
'    #MENGAMBIL DF per Cohort (Partial)
    ...
    '
    #MENGAMBIL DF Compiled
    data_comp_P <- read_excel("Workbook/OSClaim-Net.Prior.xlsx", sheet = "PA")
    ...
    '    #MENGAMBIL DF per Cohort (Partial)
    path_name <- paste0("Workbook/PC ",cohort,st_or_lt,"/OSClaim-Net.Raw.xlsx")
    data_par_PR <- read_excel(path_name, sheet = "PA-Retail")
    data_par_PR <- as.data.frame(lapply(data_par_PR, function(x) {
      x[is.na(x)] <- 0
      return(x)
    ...
    '
    #MENGAMBIL DF Compiled
    data_comp_V <- read_excel("Workbook/OSClaim-Net.Prior.xlsx", sheet = "Various")
...
'    #MENGAMBIL DF per Cohort (Partial)
    ...
    #MENGAMBIL DF Compiled
    data_comp <- read_excel("Workbook/OSClaim-Net.Prior.xlsx", sheet = sheet_name)
...
for(k in cohort_start:cohort_end){
  ...
  for(l in 0:1){
    ...
    file_path <- paste0("Workbook/OS ", k, st_or_lt, "/OSClaim-Net.Ratio.xlsx")
    wb <- createWorkbook(file_path) 
    
    for(m in sheet_data$SheetName){
      
      data_ratio_x <- OS_prior_function(k,l,m)

...
function_raw <- function(sheet_name, condition){
  ...
  if(sheet_name == "Marine"){
    ...
    path_tot <- paste0("Workbook/PaidClaim-Net", condition)
...
for (p in sheet_data$SheetName){
  ...
  data_prior_w_ML <- read_excel("Workbook/PaidClaim-Net.Prior.xlsx", sheet = p)
...
saveWorkbook(wb, "Workbook/PaidClaim-Net.Ratio.wo.ML.xlsx", overwrite = TRUE)
saveWorkbook(wbx, "Workbook/PaidClaim-Net.Prior.wo.ML.xlsx", overwrite = TRUE)

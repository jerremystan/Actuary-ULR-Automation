...
Triangle_total_raw_function <- function(sheet){
  triangle_temp <- read_excel(paste0("Workbook/","/PaidClaim-Gross.Total.Raw.xlsx"), sheet = sheet, range = excel_range)
  for (x in 1:row_count){
    if (x > 1){
      triangle_temp[x,(row_count+2+2-x):(row_count+2)] <- NA
    }
...
for ( i in cohort_start:cohort_end){ #COHORT
  ...
  #   
    ...
    prior_auto_st_final <- prior_auto_st %>%
    ...
    prior_auto_lt_final <- prior_auto_lt %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_auto_PL_st_final <- prior_auto_PL_st %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_auto_PL_lt_final <- prior_auto_PL_lt %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_auto_TLM_st_final <- prior_auto_TLM_st %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_auto_TLM_lt_final <- prior_auto_TLM_lt %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_auto_TLN_st_final <- prior_auto_TLN_st %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_auto_TLN_lt_final <- prior_auto_TLN_lt %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_auto_TP_st_final <- prior_auto_TP_st %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_auto_TP_lt_final <- prior_auto_TP_lt %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_eng_st_final <- prior_eng_st %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_eng_lt_final <- prior_eng_lt %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_fire_st_final <- prior_fire_st %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_fire_lt_final <- prior_fire_lt %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_marine_st_final <- prior_marine_st %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_marine_lt_final <- prior_marine_lt %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_various_st_final <- prior_various_st %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_various_lt_final <- prior_various_lt %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_liab_st_final <- prior_liab_st %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_liab_lt_final <- prior_liab_lt %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_health_st_final <- prior_health_st %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_health_lt_final <- prior_health_lt %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_vare_st_final <- prior_vare_st %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_vare_lt_final <- prior_vare_lt %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_firexl_st_final <- prior_firexl_st %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))
    ...
    prior_firexl_lt_final <- prior_firexl_lt %>%
      mutate(across(everything(), ~ ifelse(is.nan(.), 0, ifelse(is.infinite(.),0,.))))

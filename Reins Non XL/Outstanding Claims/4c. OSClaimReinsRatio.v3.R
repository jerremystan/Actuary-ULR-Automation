...
OS_prior_function <- function(cohort, term, sheet_name){
  ...
  if(sheet_name == "Marine"){
    ...
    path_name <- paste0("Workbook/PC ",cohort,st_or_lt,"/OSClaim-Reins.Raw.xlsx")
    ...
    data_comp_M <- read_excel("Workbook/OSClaim-Reins.Prior.wo.ML.xlsx", sheet = "Marine")
    data_comp <- data_comp_M
    
    #MENGAMBIL DF Total (semua cohort)
    path_tot <- "Workbook/OSClaim-Reins.Total.Raw.xlsx"
    ...
    '
    #MENGAMBIL DF Compiled
    data_comp_P <- read_excel("Workbook/OSClaim-Reins.Prior.wo.ML.xlsx", sheet = "PA")
    ...
    '
    #MENGAMBIL DF Compiled
    data_comp_V <- read_excel("Workbook/OSClaim-Reins.Prior.wo.ML.xlsx", sheet = "Various")
    ...
    #MENGAMBIL DF Compiled
    data_comp <- read_excel("Workbook/OSClaim-Reins.Prior.wo.ML.xlsx", sheet = sheet_name)
...
for(k in cohort_start:cohort_end){
  ...
  for(l in 0:1){
    ...
    file_path <- paste0("Workbook/OS ", k, st_or_lt, "/OSClaim-Reins.Ratio.xlsx")

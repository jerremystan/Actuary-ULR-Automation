...
function_raw <- function(sheet_name, condition){
  ...
  if(sheet_name == "Marine"){
    ...
    path_tot <- paste0("Workbook/OSClaim-", condition)
...
for (p in sheet_data$SheetName){
  ...
  data_prior_w_ML <- read_excel("Workbook/OSClaim-Gross.Prior.xlsx", sheet = p)
...
saveWorkbook(wb, "Workbook/OSClaim-Reins.Ratio.wo.ML.xlsx", overwrite = TRUE)
saveWorkbook(wbx, "Workbook/OSClaim-Reins.Prior.wo.ML.xlsx", overwrite = TRUE)

...
function_raw <- function(sheet_name, condition){
  ...
  if(sheet_name == "Marine"){
    ...
    path_tot <- paste0("Workbook/PaidClaim-", condition)
...
for (p in sheet_data$SheetName){
  print(p)
  data_tot_wo_ML <- function_raw(p, "Reins.Total.Raw.xlsx")
  #data_tot_wo_ML <- data_tot__wo_ML %>%
  #mutate(across(everything(), ~ ifelse(is.infinite(.), 0, .)))
  
  data_tot_w_ML <- function_raw(p, "Gross.Raw.xlsx")
  #data_tot_w_ML <- data_tot_w_ML %>%
  #mutate(across(everything(), ~ ifelse(is.infinite(.), 0, .)))
  
  data_prior_w_ML <- read_excel("Workbook/PaidClaim-Gross.Prior.xlsx", sheet = p)
...
saveWorkbook(wb, "Workbook/PaidClaim-Reins.Ratio.wo.ML.xlsx", overwrite = TRUE)
saveWorkbook(wbx, "Workbook/PaidClaim-Reins.Prior.wo.ML.xlsx", overwrite = TRUE)
